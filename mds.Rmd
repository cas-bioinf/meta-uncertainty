...
- DistatisR umí MDS nad množinou matic
...
plot_sampled_mds <- function(base_mds, aligned_samples, mapping) {
  ...
  rbind(base_points, sampled_points) %>%
    ...
    ggplot(aes(MDS1, MDS2, color = SampleType, shape = Soil, size = is_original, alpha = is_original)) +
...
aligned_samples_rarefy <- metaMDS_per_sample(samples_rarefy) %>% purrr::map(vegan::procrustes, X = base_mds)
plot_sampled_mds(base_mds, aligned_samples_rarefy, mapping)
```
...
plot_sampled_mds(base_classical_mds, aligned_samples_classical_rarefy, mapping)
```
...
resampled_otu <- sample_posterior_dm_all(20, t_otutab, prior = "ml", N_draws = "original")
...
```{r}
plot_sampled_mds(base_mds, resampled_mds_aligned, mapping)
```
...
resampled_otu_filtered <- sample_posterior_dm_all(20, t_otutab_filtered, prior = "ml", N_draws = "original")
...
```{r}
plot_sampled_mds(base_mds_filtered, resampled_mds_filtered_aligned, mapping)
```
...
plot_sampled_mds(base_mds, resampled_mds_aligned_deseq, mapping)
```
...
plot_sampled_mds(base_classical_mds, classical_mds_deseq, mapping)
```
