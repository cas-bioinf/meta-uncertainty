...
```{r}
experiment_dunthorn <- eval_mds_experiment(dunthorn_run_file, Origin)
```
...
```{r}
experiment_dunthorn$global %>% filter(consistency_distances < 0.6)
plot(experiment_dunthorn$sens_checks[[32]], color_aes = Origin, shape_aes = Origin)
...
```{r}
experiment_geel$per_point %>% my_plot(location, squared_error_location, n_observations)
