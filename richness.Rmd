...
```{r}
otutable = read.delim(file = here("tijana",'Seedlings_ITS2_otutab_ALL.csv'), sep = "\t") #reading the data I want to use
otutabP = otutable[, -1]
rownames(otutabP) = otutable [, 1]

mapping = read.delim(file = here("tijana","Seedlings_ITS2_year1_mapping.csv"), sep = ",") %>%
...
alpha_ <- alpha_guess_ml(observed)

xx <- rdirichlet_multinomial(100, sum(observed), rep(alpha_, length(observed)))
rowSums(xx > 0)


dirichlet_multinomial_lpmf(observed, rep(alpha_, length(observed)))
dirichlet_multinomial_lpmf(observed, rep(alpha_+ 1e-5, length(observed)))
dirichlet_multinomial_lpmf(observed, rep(0.1, length(observed)))

dirichlet_multinomial_lpmf_dalpha(observed, 1e-12)
dirichlet_multinomial_lpmf_dalpha(observed, 0.5)
ff <- Vectorize(dirichlet_multinomial_lpmf_dalpha, "alpha")
# ff <- Vectorize(function(observed, alpha) { dirichlet_multinomial_lpmf(observed, rep(alpha, length(observed)))} , "alpha")
curve(ff(observed, x), from = 1e-12, to = 2)

```


```{r}
resampled_otu <- sample_posterior_dm_all(100, as.matrix(otutabP), prior = "empirical") 
...
resampled_richness %>% t() %>% as.data.frame() %>%
...
```{r}
resampled_otu <- sample_posterior_dm_all(100, t(as.matrix(otutabP)), prior = "empirical") 
...
plot_shannon = ggplot(shannon3, aes(x = SampleType, y = shannondiv, fill = Soil)) +
