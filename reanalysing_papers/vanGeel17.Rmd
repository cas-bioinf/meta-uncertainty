...
```{r}
groups <- data.frame(observation = rownames(base_mds$points)) %>% inner_join(mapping, by = c("observation" = "Sample")) %>% pull("type")

connectivity_group(base_mds, mds_samples_dm)
connectivity_all_groups(base_mds, mds_samples_dm, groups = groups)
```
