...
plot_samples_with_orig(resampled_mds_ml)
```
...
plot_samples_with_orig(resampled_mds_deseq)
```
...
plot_samples_with_orig(classical_mds_deseq)
```
```{r}
set.seed(321485246)
#resampled_otu <- sample_posterior_dm_all(10, t_otutab, prior = 0.5, N_draws = "original") 
...
set.seed(13584224)
#resampled_otu <- sample_posterior_dm_all(10, t_otutab, prior = 0.5, N_draws = "original") 
resampled_otu_deseq_rep <- sample_posterior_DESeq2(20, t_otutab,  mapping, ~Replicate)
...
plot_samples_with_orig(resampled_mds_deseq_rep)
```
...
plot_samples_with_orig(classical_mds_deseq_rep)
```
...
head(coef(dds))
res1 <- results(dds, contrast = c("SoilSample","Beech_B","Beech_R"))
head(res1)

res2 <- results(dds, contrast = c("SoilSample","Beech_R","Beech_B"))
head(res2)
